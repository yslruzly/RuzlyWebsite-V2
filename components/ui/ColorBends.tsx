...
// The flowing color background in the hero. It's the "Color Bends" shader
// from reactbits.dev, adapted a bit for this site: the pointer listener is
// on window (so it still reacts while your cursor is over the hero text),
// and reduced motion gets a single still frame instead of the animation.
// It draws bands of the colors you pass in, warped around each other and
// bending toward the cursor.
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      // reduced motion only draws on demand, so redraw after a resize
    ...
    if (prefersReduced) {
      ...
      const rad = ((rotationRef.current % 360) * Math.PI) / 180;
  ...
  // keep the shader uniforms in sync when props change
  useEffect(() => {
  ...
  // track the cursor on window, not the container. the container sits behind
  // the hero text with pointer-events off, so it would never hear anything
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
